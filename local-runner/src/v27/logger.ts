    ...
    if (this.supabase) {
      try {
        await this.supabase.from('v27_evaluations').insert({
          ...
          asset: log.asset,
          strike_price: log.strikePrice,
          time_remaining: log.timeRemainingSeconds,
          spot_price: log.spotPrice,
          spot_ts: log.spotTs,
          delta_abs: log.deltaAbs,
          delta_pct: log.deltaPct,
          threshold: log.threshold,
          up_mid: log.upMid,
          down_mid: log.downMid,
          polymarket_ts: log.polymarketTs,
          spread_up: log.spreadUp,
          spread_down: log.spreadDown,
          causality_pass: log.causalityPass,
          spot_lead_ms: log.spotLeadMs,
          mispricing_exists: log.mispricingExists,
          mispriced_side: log.mispricedSide,
          expected_price: log.expectedPolyPrice,
          actual_price: log.actualPolyPrice,
          price_lag: log.priceLag,
          confidence: log.confidence,
          filter_pass: log.filterPass,
          failed_filter: log.failedFilter,
          aggressive_flow: log.aggressiveFlowMetrics,
          book_shape: log.bookShapeMetrics,
          spread_expansion: log.spreadExpansionMetrics,
          decision: log.decision,
          reason: log.reason,
          order_side: log.orderSide,
          order_price: log.orderPrice,
          order_shares: log.orderShares,
        });
      } catch (err) {
